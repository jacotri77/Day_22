...
return phone.match(/\(?(\d{3})?\)?[\.\-\ ]?(\d{3})[\.\-\s ]?(\d{4})/)
}
...
return number.match(/\b[0-1]{1,}0\b/)
}
...
return str.match(/\b[0-9 a-f]+[0-9 a-f]/)
}
...
return str.match(/\$\b\d+[\,\.]?\b\d{0,3}[\.\,]?\b\d{0,3}[\.\,]?\b\d{0,3}/)
}
...
function testZip (str) {
 return str.match(/(\b\d{5}\b-\d{4}\b)|(\b\d{5}\b$)/)
}
